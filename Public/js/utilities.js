...
function isThereAnyCoinChangeInMachine(amount, itemNumber) {
    ...
    for (let amountOfChange = 1; amountOfChange <= change; ++amountOfChange) {
        ...
        for (let i = 0; i < coins.length; i++) {
            if (amountOfChange - coins[i] >= 0) {
                if (min_coins[amountOfChange - coins[i]] + 1 <
...
function getChangeForCash(amount, itemNumber) {
    ...
    globalVariable.stringChange = changeString.substring(0, changeString.length - 3);
...
export { fillItemLeft, updateMachineCash, getChangeForCash, getChangeForCredit, reset, update, disableCash, enableCash, disableCredit, enableCredit, hideCredit, toFixed, isThereAnyCoinChangeInMachine, addToLogs, };
