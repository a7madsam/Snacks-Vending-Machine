...
globalVariable.coins.push(new Coin(Denomination.tenCent, 1000), new Coin(Denomination.twentyCent, 500), new Coin(Denomination.fiftyCent, 200), new Coin(Denomination.oneDollar, 100), new Coin(Denomination.twentyDollar, 5), new Coin(Denomination.fiftyDollar, 2));
...
function sumInsertedCoin(event) {
    if (startVendingMachine.getState instanceof NoMoneyState) {
        ...
        globalVariable.totalAmountOfInsertedCoins += utilities.toFixed(Number(event.target.getAttribute("data-money")), 2);
        utilities.updateMachineCash(event.target, 1);
        domElement.screenPanel.innerHTML = `You insert ${utilities.toFixed(globalVariable.totalAmountOfInsertedCoins, 2)}$, please select item to buy or ESC to cancel`;
...
function deleteCharacterFromScreenPanel() {
    ...
    let newString = globalVariable.selectedProduct.substring(0, globalVariable.selectedProduct.length - 1);
...
function collectCashAndAcceptItem() {
    let moneyInserted = false;
    let itemNumberInserted = false;
    let itemNumberTrue = false;
    let validatePrices = false;
    let itemSoldOut = true;
    ...
    if (!validatePrices) {
        domElement.screenPanel.innerHTML = `your inserted money is not enough to buy selected item so insert ${utilities.toFixed(globalVariable.prices()[Number(globalVariable.selectedProduct) - 1] -
            globalVariable.totalAmountOfInsertedCoins, 2)} or more to buy it or ESC to cancel`;
        utilities.addToLogs(domElement.screenPanel.innerHTML);
        return;
    }
    //check if machine have any coin to give change
    if (globalVariable.typeOfPayment === PaymentType.cashPayment &&
        utilities.isThereAnyCoinChangeInMachine(globalVariable.totalAmountOfInsertedCoins, Number(globalVariable.selectedProduct)) === -1) {
...
domElement.insertionAreaDeleteButton.addEventListener("click", deleteCharacterFromScreenPanel);
